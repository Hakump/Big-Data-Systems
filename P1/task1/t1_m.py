...
conf = SparkConf().setAppName("task1").setMaster("spark://10.10.1.1:7077") \
...
links = lines.map(lambda line: tuple(re.split("\\t", line.lower(), maxsplit=1))) \
             .filter(lambda x: ftl(x)).map(lambda x: (x[0], [x[1]]))\
            .reduceByKey(lambda x, y: x + y).cache()  # not (":" in x) or ("category" in x)
...
for i in range(ITERATION_NUM):
    ...
    targetSum = linkRankRDD.flatMap(
        lambda pair: averageRank(pair[0], pair[1])) \
        .reduceByKey(lambda x, y: x + y)  # get the list of (target, rank) pair to increase the rank
    # sum up the rank for each target

    ranks = targetSum.mapValues(lambda x: 0.15 + 0.85 * x)  # assign the new rank

ranks.saveAsTextFile("hdfs://10.10.1.1:9000/task1Out")  # save to output to local
