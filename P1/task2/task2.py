...
conf = SparkConf().setAppName("task2").setMaster("spark://10.10.1.1:7077") \
...
links = links.repartition(PARTITIO_NUM) 
ranks = links.map(lambda pair: (pair[0], 1.0) ,  preservesPartitioning=True)  # create the ranks <key,one> RDD from the links <key, Iter> RDD

for i in range(ITERATION_NUM):
    linkRankRDD = links.join(ranks,numPartitions=PARTITIO_NUM)  # joined result should be source : (targets, rank)
    # get the list of (target, rank) pair to increase the rank
    targetSum = linkRankRDD.flatMap(lambda pair: averageRank(pair[0], pair[1]), preservesPartitioning=True) \
                           .reduceByKey(lambda x, y: x + y, numPartitions=PARTITIO_NUM)  # sum up the rank for each target
    ranks = targetSum.mapValues(lambda x: 0.15 + 0.85 * x)  # assign the new rank

ranks.saveAsTextFile("hdfs://10.10.1.1:9000/task2Out")  # save to output to local
